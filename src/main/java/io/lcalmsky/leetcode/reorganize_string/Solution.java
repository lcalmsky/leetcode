...
public class Solution {
    public String reorganizeString(String S) {
        ...
        for (char c : map.keySet()) {
            if ((len % 2 == 0 && map.get(c) > len / 2) || (len % 2 != 0 && map.get(c) >= len / 2 + 2)) return "";
        }
        PriorityQueue<CustChar> queue = new PriorityQueue<>((a, b) -> b.count - a.count);
        for (char c : map.keySet()) queue.offer(new CustChar(c, map.get(c)));
        ...
        while (!queue.isEmpty()) {
            ...
            if (sb.length() > 0 && sb.charAt(sb.length() - 1) != c) {
                sb.append(c);
                if (curr.count > 1) queue.offer(new CustChar(c, curr.count - 1));
            } else if (sb.length() == 0) {
                sb.append(c);
                if (curr.count > 1) queue.offer(new CustChar(c, curr.count - 1));
                ...
                sb.append(next.c);
                if (next.count > 1) queue.offer(new CustChar(next.c, next.count - 1));
                queue.offer(curr);
            }
        }
        return sb.toString();
